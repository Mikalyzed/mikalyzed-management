...
- `VehicleStage` — one row per stage visit; `checklist Json` items shaped `{item, done, note, type?, fields?, data?, addedByMechanic?, approved?, assigneeId?, assigneeName?, fromPart?, fromPlan?, doneAt?, mechanicFlag?}`. `doneAt` is stamped **server-side** in the stage PATCH on the transition to done (since 2026-07-28; earlier completions have no timestamp). `scopeName` labels templated stages ("New Vehicle Inspection"). `timers Json` is the per-mechanic clock map keyed by userId (`{activeSeconds, timerStartedAt, done, doneAt, …}`) — on a shared car, a mechanic "Finish My Part" stamps their `done`/`doneAt` and the card moves into **their** Done Today / Completed This Week (viewer-scoped) while co-workers keep going.
...
- `ExternalRepair` — statuses `pending → sent → in_progress → ready → returned`. `pending` means **planned, car still on the lot** (it must not flip the car to at-external — `lib/inventory-status.ts` enforces this). `partOnly` = the component went out, not the car. `plannedSendDate` = when it's supposed to leave (a passed date triggers alerts). `installPartId` (added 2026-08-04) = a RECEIVED part sent OUT for work (e.g. upholstery) that installs **in-house on return** — distinct from `installVenue:'external'` (shop installs it on the car); marking such a repair `returned` queues an `Install: <part>` task onto the car's mechanic recon (`lib/part-install.ts`). Marking a repair away from `ready` deletes its open "pick it up" retrieve task (keeps the coordinator board synced). `followUps Json` entries are `{date, note, by, etaDays, calculatedDeadline}` — `by` (author) and `etaDays` (days added vs the prior expected date) are stamped server-side since 2026-07-30; earlier entries are anonymous.
...
`app/api/tasks/assess/route.ts` + `SmartTaskModal`. Natural language in → the AI classifies the execution shape and the server resolves the car (stock match or fleet word-match; ambiguity returns a tap-to-answer question, e.g. three Grand Nationals) and assignee (named person, else **the acting user** who added it — so a task lands on the adder's own Assignments; fixed 2026-08-04, was "first coordinator"). Four shapes:
...
**Can:** source parts (paste link → admin approval; or "In Store" purchase with receipt/price), add parts and tasks anywhere, browse all mechanic lanes and check off tasks, **assign individual mechanic tasks** (per-task "Assign to" — granted 2026-08-03), assign installs, create/mark install tasks, run missions (create tows, mark externals sent/ready/returned, log follow-ups), use `+ Add Task`, see Issues Detected, **advance a car out of the mechanic stage** (the "Advance ▸" button → `coordinator_advance`: park in Pending Routing, or propose a next stage that admin one-tap approves — granted 2026-08-03), **Send Out for Repair** (a received part with no install plan → creates a `partOnly` ExternalRepair, scheduled/pending, that installs in-house on return — 2026-08-04), and **request routing**: marking a car Returned opens the full Send-to-Recon modal (stage + tasks + templates + hours) whose primary button files the package as a `routingProposal` instead of routing.
...
- **No default mechanic (changed 2026-08-03).** The mechanic `StageConfig.defaultAssigneeId` was cleared — routing no longer auto-assigns a mechanic; assign explicitly on the routing card / Assign-the-Install queue. Unassigned original tasks read as "needs assignment" instead of silently falling to a default owner.
- **Flag for Mechanic (2026-08-03).** On a car mid-stage (e.g. a leak spotted during Content) the recon detail modal has a "Flag for Mechanic" action: it records the issue on the current stage as a resolved marker (`mechanicFlag`) WITHOUT moving the car. When the car finishes and hits routing, `extractIssueFixTasks` (`lib/inspection-issues.ts`) detects it → the route modal defaults to Mechanic and pre-fills a `Fix: <issue>` task. Preferred over yanking the car mid-stage.
- **Return-queue is honored on mechanic completion (2026-08-04).** A car kicked back to mechanic mid-stage (drag-to-lane with "return after", or the coordinator advance) now resumes its prior stage on mechanic-board Complete / `coordinator_advance`, instead of always parking in routing.
