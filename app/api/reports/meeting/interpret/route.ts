...
function validStep(s: PlanStep, hasStage: boolean): boolean {
  switch (s.type) {
    ...
    case 'external': return !!s.work?.trim()
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      system:
        ...
        `Use ONLY what the admin actually wrote — never invent details, shops, parts, or dates. ` +
        ...
        `Office work (calls, titles, quotes, scheduling, paperwork) = followup. Parts to buy = part_request.`,
    ...
    const steps = raw.filter(s => validStep(s, hasStage)).slice(0, 3)
    if (steps.length === 0) {
